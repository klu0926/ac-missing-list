...
const control = {
  ...
  getDataAndRenderPage() {
    axios.get(API)
      .then(response => {
        ...
        // 3. render lookout page 1******
        this.renderPage(1, model.lookoutData)
      })
      .catch(error => {
        console.log(error)
      })
  ...
  renderPage(page, data) {
    ...
    view.renderPagination(model.lookoutData)
  ...
  onSearchButtonClick(event) {
    ...
    if (!inputValue || inputValue.length === 0) {
      control.renderPage(1, model.lookoutData)
      view.renderResultNumber(model.lookoutData)
      view.renderLookoutCardDisplay(model.lookoutData)
      view.renderPagination(model.lookoutData)
    } else {
      // 有輸入
      const filterData = model.filterSearchData(inputValue, model.lookoutData)
  ...
  onLookoutBtnClick(event) {
    ...
    if (target.matches(".btn-primary")) {
      ...
      view.toggleLookoutBtnDisplay("lookout")
      control.renderPage(currentPage, model.lookoutData)
      view.renderLookoutNumber(model.lookoutData.length)
    }
  },
